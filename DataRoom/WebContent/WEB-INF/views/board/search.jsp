...
<body>
		...
		<div id="fh5co-main">
			<div class="fh5co-narrow-content">
				...
				<div class="row row-bottom-padded-md">
					<div class="btn-div">
						<span class="keyword-span"><b>'<c:if test='${type eq "hashtag"}'>#</c:if>${keyword}'</b>의 검색 결과입니다. (${searchCnt} 건)</span>
						<div class="btn-group btn-group-sm" role="group"
							aria-label="Basic example">
							<button type="button" class="btn active" id="am10" onclick="setAmount(10)">10개</button>
							<button type="button" class="btn " id="am20" onclick="setAmount(20)">20개</button>
							<button type="button" class="btn" id="am30" onclick="setAmount(30)">30개</button>
							<button type="button" class="btn" id="am40" onclick="setAmount(40)">40개</button>
					...
					<div class="col-padding animate-box board-container"
						...
						<div class="container show-grid board-table">
							...
							<c:forEach items="${list}" var="board">
								...
								<div class="row board-body">
									...
									<div class="col-md-4">
										<a href="/board/read?bno=${board.bno}&page=${pm.paging.page}&amount=${pm.paging.amount}&type=${type}&keyword=${keyword}">
											${board.title}
											<!-- 파일이 1개 이상이면 첨부파일 표시 -->
											<c:if test="${board.filecnt + 0 > 0}">
							...
							<ul class="pagination">
								<c:if test="${pm.prev}">
									<li class="page-item"><a class="page-link"
										href="/board/search?page=${pm.start-1}&amount=${pm.paging.amount}&type=${type}&keyword=${keyword}">«</a>
									</li>
								</c:if>
								<c:forEach begin="${pm.start}" end="${pm.end}" var="num">
									<li class='page-item ${pm.paging.page == num ? "active" : ""}'>
										<a class="page-link"
										href="/board/search?page=${num}&amount=${pm.paging.amount}&type=${type}&keyword=${keyword}">${num}</a>
									</li>
								</c:forEach>
								<c:if test="${pm.next}">
									<li class="page-item"><a class="page-link"
										href="/board/search?page=${pm.end+1}&amount=${pm.paging.amount}&type=${type}&keyword=${keyword}">»</a>
									</li>
								</c:if>
							</ul>
						</div>
						...
						<div class="form-group search-div">
							<form action="/board/search" method="GET">
								<label for="searchSelect">검색</label>
								<select
									class="form-control search-select" id="searchSelect"
									name="type">

									<c:choose>
										<c:when test='${param.type eq "title"}'>
											<c:set var="title" value="selected"/>
										</c:when>
										<c:when test='${param.type eq "writer"}'>
										    <c:set var="writer" value="selected"/>
										</c:when>
										<c:when test='${param.type eq "hashtag"}'>
										    <c:set var="hashtag" value="selected"/>
										</c:when>
									</c:choose>
									<option value="title" <c:out value="${title}"/>>title</option>
									<option value="writer" <c:out value="${writer}"/>>writer</option>
									<option value="hashtag" <c:out value="${hashtag}"/>>hashtag</option>
	...
	function setAmount(index) {
		location.href='../board/search?page=${param.page}&amount='+index+'&type=${type}&keyword=${keyword}';
	}
	</script>
</body>
</html>
